...
function _generateRootComponent(subPath, permalink, title, componentName) {
  return `
${lib._generateOneImport(componentName, componentName, "svelte")}
${lib._generateTwoImport("sveltePress", subPath, ".", "sveltePress", "js")}
...
function _generateOneImport(object, fileName, extension) {
  const fullName = _.compact([fileName, extension]).join(".");
  return `import ${object} from './${fullName}'`;
}

function _generateTwoImport(object, subPath, subPath2, fileName, extension) {
  const fullName = _.compact([fileName, extension]).join(".");
  const fullPath =
    subPath === "."
      ? _.compact([subPath2, fullName]).join("/")
      : _.compact([
          lib._generateSubPath(subPath),
          subPath2 === "." ? null : subPath2,
          fullName,
        ]).join("/");
